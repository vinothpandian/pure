...
prompt_pure_state_setup() {
    ...
    [[ -n $ssh_connection ]] && username='%F{251}%n@%m%f'
    
    # show username@host if root, with username in white
    [[ $UID -eq 0 ]] && username='%F{white}%n%f%F{251}@%m%f'
...
prompt_pure_setup() {
    ...
    ps4_parts=(
        ...
        main      '%F{green}${${(%)prompt_pure_debug_depth[3]}:t}%f%F{251}:%I%f %F{251}@%f%F{green}%N%f%F{251}:%i%f'
        secondary '%F{green}%N%f%F{251}:%i'
        prompt 	  '%F{251}>%f '
    )
